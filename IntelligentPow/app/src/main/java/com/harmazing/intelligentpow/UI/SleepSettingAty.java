...
public class SleepSettingAty extends Activity {
    ...
    private RelativeLayout btnTimePicker,btnDayPicker,btnDelete;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sleep_setting_aty);
        //界面图标
        findViews();
...
//            getStateFromAC();//添加，现在的空调状态
    ...
    private void getData() {
        ...
        RelativeLayout.LayoutParams pl  = (RelativeLayout.LayoutParams) btnTimePicker.getLayoutParams();
        pl.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
        btnTimePicker.setLayoutParams(pl);
        btnState.setCheck(true);
        nowAlertItem.getClockSetting().setOn_off(OPEN);

        Bundle bundle = this.getIntent().getExtras();
        final String curveId = bundle.getString("curveId");;//临时用传curveId
        final String deviceId = bundle.getString("deviceId");;//临时用传curveId
        if(null != curveId && !"".equals(curveId)){
            nowAlertItem.setRairconSetting(curveId);
        }
    ...
    public void findViews(){
        ...
        btnDelete = (RelativeLayout) findViewById(R.id.delet);
        //时间，周期
    ...
    public void setOnclickLisenter(){
        ...
        btnConfirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (nowAlertItem.getClockSetting().getOn_off() == CLOSE){
                    nowAlertItem.getClockSetting().setTemp(15+"");
                    nowAlertItem.getClockSetting().setMode(0);
                    nowAlertItem.getClockSetting().setWindspeed(0);
                }
                String jsonString = GsonUtil.bean2Json(nowAlertItem);
                LogUtil.v("jsonString", HttpHead.head + API.ADD_CURVES_CLOCKS + jsonString);
                if (!txTime.getText().toString().equals("未设置") && !txTime.getText().toString().equals("")){
                    LogUtil.v("httpPosturl", HttpHead.head + API.ADD_CURVES_CLOCKS);
                    Toast.makeText(SleepSettingAty.this, "发送。。。", Toast.LENGTH_LONG).show();
                    if (from == ADD)
                        httpPost(HttpHead.head + API.ADD_CURVES_CLOCKS, jsonString);
                    else{
                        httpPost(HttpHead.head + API.UPDATE_SLEEP_ALERT, jsonString);
                        nowAlertItem.getClockSetting().setStartend(1);
                    }
//                    finish();
                }else{
                    Toast.makeText(SleepSettingAty.this, "请设置一个时间", Toast.LENGTH_LONG).show();
                }
        ...
        btnDelete.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                final AlertDialog alertDialog = new AlertDialog.Builder(SleepSettingAty.this).create();
                alertDialog.setMessage("确定要删除该点设置么？");
                alertDialog.setButton(DialogInterface.BUTTON_POSITIVE,"确定",new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialogInterface, int i) {
                        deleteItem(alertItem);
                    }
                });
    ...
    private void deleteItem(AlertItem alertItem) {
       final ProgressDialog progressDialog = new ProgressDialog(this);
        ...
        HttpUtil.post(HttpHead.head+API.DEL_TIMESETTING,params,new JsonHttpResponseHandler(){
            ...
            public void onStart() {
                super.onStart();
                if (!SleepSettingAty.this.isFinishing()){
                    progressDialog.show();
                }
            }
            @Override
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                ...
                Toast.makeText(SleepSettingAty.this,"删除成功！",Toast.LENGTH_LONG).show();
            }
    ...
    private void httpPost(String s, String jsonString) {
        LogUtil.v("http post",s+"@@"+jsonString);
        HttpUtil.post(SleepSettingAty.this, s, jsonString, new JsonHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                super.onSuccess(statusCode, headers, response);
...
//        type = bundle.getInt("type");
    ...
    public void getStateFromAA(){
        alertItem = (AlertItem) getIntent().getExtras().getSerializable("alertItem");
    ...
    public void showTimePickerDialog() {
        ...
        final TimePickerDialog dialog = new TimePickerDialog(this, new TimePickerDialog.OnTimeSetListener() {
            ...
            public void onTimeSet(TimePicker arg0, int hour, int minute) {
                ...
                nowAlertItem.getClockSetting().setClocking(hour + ":" + minute);
                txTime.setText(hour + ":" + minute);
